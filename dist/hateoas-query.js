...
function baseHas(object, key) {
  return object != null && hasOwnProperty$8.call(object, key);
}
...
function query (node, selector, options, results) {
  ...
  options = assignIn$1({}, { request: this.request, strict: this.strict }, options);
  return queryIsolated(node, selector, options, results)
    .then(function (result) { return extended(concatReducer(result)); })
}
...
function queryIsolated (node, selector, options, results) {
  ...
  if (this) { options = assignIn$1({}, { request: this.request, strict: this.strict }, options); }
  if (!selector) {
    var only = get(options, 'only');
    if (!only) {
  ...
  var links = selector.split('.');
  var path = links.shift();
  var isIterable = isSelectorIterable(path);
  var isAction = isSelectorAnAction(path);
  var remainingSelector = links.join('.');
  var requestOptions;
  var actionParams;
  if (isAction) {
  ...
  if (!(requestOptions.path || requestOptions.action)) {
    ...
    var item = getAttribute(node, path);
    if (item !== undefined) {
  ...
  return options.request(
    assignIn$1({}, requestOptions, { params: actionParams })
  )
    .then(function (response) {
      ...
      if (!isIterable) {
        ...
        var filteredItems = isSelectorFiltered(path) ? items[getSelectorFilter(path)] : items;
...
function extended (obj) {
  ...
  if (!isArray) {
    obj.get = function () {
      ...
      return extended(get.apply(void 0, [ obj ].concat( args )));
    };
    obj.has = function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];

      return extended(has.apply(void 0, [ obj ].concat( args )));
    };
    obj.keys = function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];

      return extended(keys.apply(void 0, [ obj ].concat( args )));
    };
    obj.values = function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];

      return extended(values.apply(void 0, [ obj ].concat( args )));
    };
  }
  // collection
  if (isArray) {
    obj.find = function () {
      ...
      return extended(find.apply(void 0, [ obj ].concat( args )));
    };
    obj.map = function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];

      return extended(map.apply(void 0, [ obj ].concat( args )));
    };
    obj.filter = function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];

      return extended(filter.apply(void 0, [ obj ].concat( args )));
    };
    obj.reduce = function () {
      var args = [], len = arguments.length;
      while ( len-- ) args[ len ] = arguments[ len ];

      return reduce.apply(void 0, [ obj ].concat( args ));
    };
  }
  return obj
}

var getLink = function (node, path) { return get(node, ("_links." + (path.replace(/\[\d*]$/, '')) + ".href")); };
var getAction = function (node, path) { return get(node, ("_actions." + (path.replace(/^@/, ''))), {}); };
var getAttribute = function (node, path) { return get(node, path); };

var isSelectorIterable = function (selector) { return /\[\d*]$/.test(selector); };
var isSelectorFiltered = function (selector) { return /\[\d+]$/.test(selector); };
var isSelectorAnAction = function (selector) { return /^@/.test(selector); };
...
function concatReducer (collection) {
  if (collection && Array.isArray(collection[0])) {
    return reduce(collection, function (accumulator, current) { return accumulator && current ? accumulator.concat(current) : accumulator || [current]; }
    )
  } else {
    return collection
  }
...
var instance = function (ref) {
  ...
  return query.bind({ request: request, strict: strict });
};

// Expose some functions
instance.query = function (ref) {
  var request = ref.request;
  var strict = ref.strict;

  return query.bind({ request: request, strict: strict });
};
instance.isolated = function (ref) {
  var request = ref.request;
  var strict = ref.strict;

  return queryIsolated.bind({ request: request, strict: strict });
};
